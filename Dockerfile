...
CMD sh -c 'echo "Connecting to database at $DATABASE_HOST" 'PGPASSWORD="$DATABASE_PASSWORD" psql -h "$DATABASE_HOST" -U "$DATABASE_USER" -d "$DATABASE_NAME" -c "SET search_path TO $SCHEMA;" && flask db upgrade && flask seed all && gunicorn app:app'
